...
export abstract class PortfolioService {
	...
	static async paginatePublished(filter: PortfolioModel.PortfolioFilterDto) {
		const result = await PortfolioRepository.paginatePublished(filter);
		return {
			data: result.data,
			meta: paginationMeta(filter, result.total_count),
		};
	...
	static async create(
		userId: number,
		payload: PortfolioModel.CreatePortfolioDto,
	) {
	...
	static async update(
		id: string,
		userId: number,
		payload: PortfolioModel.UpdatePortfolioDto,
	) {
	...
	static async assignCategories(
		id: string,
		userId: number,
		payload: PortfolioModel.AssignCategoriesDto,
	) {
		await this.findOwnedById(id, userId);
		const categoryIds = uniqueCategoryIds(payload.category_ids);
		await this.assertCategoriesOwned(categoryIds, userId);
